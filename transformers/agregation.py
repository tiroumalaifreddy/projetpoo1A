...
class Agregation(Transformers):
    def transform(self, Table, index_variable_agr, variable_modifie, echelle):
        ...
        if echelle == "region":
            data_imp = [['numRegion', variable_modifie]]
            for numRegion in liste_reg:
                ...
                liste_deux = []
                for i in Table.rows:
                    if i[index_variable_agr] in liste:
                        liste_deux.append(i)
                somme = 0
                for k in liste_deux:
                    somme += float(k[Table.column_names.index(variable_modifie)])
